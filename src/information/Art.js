export const arts = [
  {
    title: 'Flowers',
    dates: 'June 2022 - February 2023',
    description: 'My favorite style that I do--this piece is 18" x 24", filled with months of doodles. I have tried over the years to keep diaries but they never last. I used to love doodling during class; I found it much easier to concentrate. So when I graduated college I felt a part of my life missing, but doodling didn\'t feel enough like "art" until I started making larger scale pieces. And now these are my most popular pieces, so take that 7th grade Algebra teacher.',
    medium: ['Markers', 'Colored Pencils', 'Oil Pastels'],
    link: 'https://drive.google.com/uc?id=1bmXhfiuqhwnGBWfY63YMCD45QO2c92dZ',
    path: 'img/Flowers.jpg'
  },
  {
    title: 'The Moirologist',
    dates: 'September 2023',
    description: 'Inspired by \'Self Portrait As A Moirologist\' by Kathryn Bratt-Pfotenhauer, this piece explores my complicated relationship with death, grief, and night. I\'ve also long loved crows and dream of building a real friendship with my local murder. Read the poem in their debut anthology, "Bad Animal"!',
    medium: ['Digital (Procreate)'],
    link: 'https://drive.google.com/uc?id=1tpzuKaDr483X5oex4uSb7727U-B6RfPi',
    path: 'img/[Bad_Animal].JPG'
  },
  {
    title: 'A Mushroom',
    dates: 'November 2021 - June 2022',
    description: 'One of my first explorations into the doodle style. This piece started as more of a proof of concept, and as it evolved I started to see the flaws in the composition and what I would want to do next. While I\'ve always prefered warm colors to cool, I chose this color scheme in honor of my partner, who the piece is for. Of course, I didn\'t finish it until we lived together, but he knows it\'s his at least.',
    medium: ['Markers', 'Graphite', 'Colored Pencils', 'Acrylic Paint'],
    link: 'https://drive.google.com/uc?id=1f83bUxbXzdbPDxmQTtYwD2ef7aHjD_9a',
    path: 'img/mushroom.jpg'
  },
  {
    title: 'Inside the Ironwood Tree',
    dates: 'January 2023',
    description: 'I saw a picture from inside an Ironwood tree on Reddit and wanted to explore the light and shadows inside a tree. A comrade was murdered by police the week I finished this piece, so it became a dedication to them and their devotion to the Welaunee Forest. More trees; fewer cops.',
    medium: ['Digital (Procreate)'],
    link: 'https://drive.google.com/uc?id=1Qw_LKlndkMLtRnjjmT5N2y05MBMhlr6v',
    path: 'img/addie-l-b-inside-the-ironwood-tree.jpg'
  },
  {
    title: 'Merry Samhain',
    dates: 'October 2022',
    description: 'An entry to Jerry\'s Artarama Nashville Halloween contest, this is a still life of my altar as it was set up for Samhain. Samhain is the Celtic/Irish "Halloween" where the death and ancestors are honored and celebrated. The mirror portal in the middle is, of course, added for style--I\'ve yet to enter any other dimensions.',
    medium: ['Oil Pastels', 'Black Canvas'],
    link: 'https://drive.google.com/uc?id=1f2_8jGsVit6gfZD8TAL85ZV2oXUAgxKZ',
    path: 'img/altar.jpg'
  },
  {
    title: 'Inner Children',
    dates: 'December 2022',
    description: 'Growing up, my favorite movie was (and honestly still is) \'My Neighbor Totoro\' from Studio Ghibli. To celebrate the art that inspired so much of my style, I redesigned the totoros in the style of mine and my sister\'s cats, and drew the two sisters as a younger version of my sister and me. I see so much of my childhood in this movie, and I love that I was able to recreate such a sweet feeling in my own life.',
    medium: ['Digital (Procreate)'],
    link: 'https://drive.google.com/uc?id=1zaRSpCHfTNnek3dLlj1e9NN6hQhk5nNJ',
    path: 'img/Catoro.jpg'
  },
  {
    title: 'Licha Moss',
    dates: 'May 2023',
    description: 'My first exploration into character design: my D&D character, Licha Moss. Licha is a proud representative of the MLM "FungaLife" which specializes in healthy shakes for druid elves. Licha came from modest beginnings as a chaotic good backwoods elf who--after a surprising encounter with a certain deck of cards--transformed into her lawful evil salesperson self we know and love today. She\'s a real #MossBabe.',
    medium: ['Digital (Procreate)'],
    link: 'https://drive.google.com/uc?id=1_6o4uCWBpeYwM4igVcsZqMFzAjiliLNd',
    path: 'img/mossbabe.jpg'
  },
  {
    title: 'Several Mushrooms',
    dates: 'February 2023 - July 2023',
    description: 'A continuation of my doodle projects, this is a focus on warm colors and differentiation as they overlap. This is also the first piece I switched to black outlines for. If it wasn\'t already obvious, mushrooms--any nature really, but specifically mushrooms--are a favorite subject of mine, and I wanted to revist the doodle mushroom concept and improve upon it using my own favorite colors.',
    medium: ['Markers', 'Colored Pencils', 'Oil Pastels'],
    link: 'https://drive.google.com/uc?id=1AfRTbUbq8llqBxMw1ZaJO_0D2V8_iol8',
    path: 'img/mushrooms.jpg'
  },
  {
    title: 'Thecius',
    dates: 'May 2023',
    description: 'My second foray into character design: this time I took on the challenge of a character I didn\'t build. My partner\'s D&D character, Thecius, is a nerdy wizard who prefers to summon elementals for his dirty work. Small brag--my partner is very witty and frequently comes up with silly names for said elementals, this one being "Liberal Tears Gas Stove" as it was a fire elemental.',
    medium: ['Digital (Procreate)'],
    link: 'https://drive.google.com/uc?id=1AyP8PXsS7eP0KPelpyf3YfkCrsA0lnkY',
    path: 'img/The_Ships_Owner.jpg'
  },
  {
    title: 'Winter Nights',
    dates: 'November 2022 - February 2023',
    description: 'I used to hate winter--the cold and dark has always unsettled me. Until Winter 2021, where I had a chill, work-from-home job (not a high bar compared to the hospitality industry I was used to) and got to indulge my human urges to be slower, quieter, calmer, and less active during the winter. I started to appreciate the early nights because it brought stars, the open window into the chill because I could be warm and still, and the quiet while the local wildlife hibernates.',
    medium: ['Acrylic Paint', 'Black Canvas'],
    link: 'https://drive.google.com/uc?id=1OFGYLF4_kAWzfDj_PbcpIRACUDG9nfIk',
    path: 'img/winterNights.jpg'
  }
]